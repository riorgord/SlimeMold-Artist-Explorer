"""
输出：FAISS 索引 + 向量矩阵 + 元数据 + PCA 聚类（和原流程产物一致）

用法：
  python scripts/build_anima_local.py \
    --md ./artists/artists40k.md \
    --encoder E:/path/to/qwen_3_06b_base.safetensors \
    --output ./outputs_anima_local \
    --prefix "1girl,"
"""
import sys, os
...
def main():
    parser = argparse.ArgumentParser(description="一键本地构建 Anima 画师向量库")
    parser.add_argument("--md", required=True, help="Markdown 画师表格路径")
    parser.add_argument("--encoder", required=True, help="Qwen3 文本编码器 .safetensors 路径")
    parser.add_argument("--output", default="./outputs_anima", help="输出目录 (默认 ./outputs_anima)")
    parser.add_argument("--prefix", default="1girl,", help="Prompt 前缀，末尾自动追加 @画师名")
    parser.add_argument("--device", default=None, help="设备 (cuda/cpu，默认自动)")
    parser.add_argument("--save-vectors", action="store_true", help="保存完整向量矩阵 .npy (可能很大)")
    ...
    # 2. 加载编码器（只加载一次）
    print(f"🔧 加载 Qwen3 编码器: {args.encoder}")
    from engines.clip_encoder_anima import AnimaEncoder
    device = args.device or ("cuda" if torch.cuda.is_available() else "cpu")
    encoder = AnimaEncoder(args.encoder, device=device)
    encoder.load()
    print(f"   设备: {device}")

    # 3. 逐画师编码
    print(f"🧬 编码中 (prefix='{args.prefix} @')...")
    vectors = []
    valid_ids = []
    valid_names = []
    failed = []
    ...
    for artist in tqdm(artists, desc="编码画师"):
        ...
        aname = artist['name']
        try:
            vec = encoder.encode(f"{args.prefix} @{aname}")
            vectors.append(vec)
    ...
    index_file = output_dir / "artist_index_anima.faiss"
    faiss.write_index(index, str(index_file))
    print(f"   💾 {index_file}")

    # 5. ID 列表
    ids_file = output_dir / "artist_ids_anima.json"
    ...
    if args.save_vectors:
        npy_file = output_dir / "artist_vectors_anima.npy"
        np.save(npy_file, vectors_matrix)
        print(f"   💾 {npy_file}")

    # 7. 元数据
    meta_file = output_dir / "artist_metadata_anima.csv"
    df_meta = pd.DataFrame({"artist_id": valid_ids, "artist_name": valid_names})
    df_meta.to_csv(meta_file, index=False, encoding='utf-8')
    print(f"   💾 {meta_file}")
    ...
    try:
        ...
        pca_file = output_dir / "artist_pca_clusters_anima.csv"
        df_pca = pd.DataFrame({"artist_id": valid_ids, "pc1": coords[:, 0], "pc2": coords[:, 1], "cluster": clusters})
        df_pca.to_csv(pca_file, index=False, encoding='utf-8')
        print(f"   💾 {pca_file}")
    except ImportError:
    ...
    # 9. library.json
    lib_json = {
        "name": f"Anima {args.prefix.strip()}",
        "arch": "anima",
        "dim": int(dim),
        "prefix": args.prefix,
    }
