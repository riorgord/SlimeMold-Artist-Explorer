# Adapted from ComfyUI (GPL-3.0) — comfy/sdxl_clip.py. See NOTICE in this directory.
import torch
...
class SDXLClipG(sd1_clip.SDClipModel):
    def __init__(self, device="cpu", max_length=77, freeze=True,
                 layer="penultimate", layer_idx=None, dtype=None, model_options={}):
        if layer == "penultimate":
            layer = "hidden"
            layer_idx = -2

        textmodel_json_config = str(Path(__file__).resolve().parent / "clip_config_bigg.json")
...
class SDXLClipGTokenizer(sd1_clip.SDTokenizer):
    def __init__(self, tokenizer_path=None, embedding_directory=None, tokenizer_data={}):
        if tokenizer_path is None:
            tokenizer_path = str(Path(__file__).resolve().parent.parent / "tokenizers" / "CLIP-ViT-bigG-14-laion2B-39B-b160k")
